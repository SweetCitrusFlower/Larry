...
def get_knowledge_sources(db: Session, user_id: Optional[int] = None, skip: int = 0, limit: int = 100) -> List[KnowledgeSource]:
    query = select(KnowledgeSource)
    if user_id is not None:
        query = query.where(KnowledgeSource.user_id == user_id)

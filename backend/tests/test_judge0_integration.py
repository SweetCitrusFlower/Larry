...
class TestJudge0Integration:
    ...
    async def test_case_1_basic_success(self, mock_get, mock_post):
        """
        Test Case 1: Submit a simple Python script and verify stdout and accepted status.
        """
        # Mock Judge0 Submission (POST)
        mock_post.return_value = httpx.Response(201, json={"token": "dummy-token-123"}, request=httpx.Request("POST", "http://test"))
        
        # Mock Judge0 Polling (GET) - use status ID 3 for 'Accepted'
        mock_get.return_value = httpx.Response(200, json={"status": {"id": 3, "description": "Accepted"}, "stdout": "Hello from Larry\n", "time": "0.1", "memory": 128}, request=httpx.Request("GET", "http://test"))
    ...
    @patch("httpx.AsyncClient.post")
    @patch("httpx.AsyncClient.get")
    async def test_case_2_input_output_logic(self, mock_get, mock_post):
        """
        ...
        # Mock Judge0 Polling (GET) - use status ID 3 for 'Accepted'
        mock_get.return_value = httpx.Response(200, json={"status": {"id": 3, "description": "Accepted"}, "stdout": "15\n", "time": "0.1", "memory": 128}, request=httpx.Request("GET", "http://test"))
...
# Non-async test for FastAPI Endpoint
    @patch("httpx.AsyncClient.post")
    @patch("httpx.AsyncClient.get")
    def test_case_3_database_persistence(self, mock_get, mock_post, client):
        """
        ...
        # Mock Judge0 Polling (GET) - use status ID 3 for 'Accepted'
        mock_get.return_value = httpx.Response(200, json={"status": {"id": 3, "description": "Accepted"}, "stdout": "DB Persistence Check\n", "time": "0.1", "memory": 128}, request=httpx.Request("GET", "http://test"))
        ...
        # This will call the real endpoint which calls submit_code which uses httpx.AsyncClient, which is patched!
